(function (root) {
  ...
  function locationSubtitle(id) {
    ...
    return treasure ? `对应镇海战备：${treasure.shortName}` : '待开放地牌';
  }

  function cardRuleText(card) {
    if (!card) return '';
    if (card.type === 'travel') return '线上规则：打出1张巡防，沿地图连线移动1格；未开放地点不能进入；本回合只能执行这1个动作。';
    if (card.type === 'inspect') return '线上规则：停留在已开放地点时打出，立即取得该地点对应镇海战备1件；库存为0时仍会消耗此牌。';
    ...
    if (card.type === 'trump') return '线上规则：用自己1件镇海战备，强制交换任意对手的1件镇海战备。';
  ...
  function renderBoardNode(locationId, point, state) {
    ...
    if (isCenter) {
      return `<div class="board-node board-node--center" data-location-id="center" style="${pointStyle}">
        <div class="board-node__title">中左所军门</div>
        <div class="board-node__subtitle">中央起点</div>
      </div>`;
    }
  ...
  function showStartDialog(hasSave) {
    ...
    refs.modalRoot.innerHTML = `<div class="modal" role="dialog" aria-modal="true">
      <h2>镇海争锋</h2>
      <p>${hasSave ? '检测到上一局存档。你可以继续游戏，或重新开局。' : '三名玩家从中左所军门出发。每回合只能执行1个动作；手牌会自动补回并尽量维持3张。先开放地牌，再巡防、探海，集齐四种不同镇海战备即可获胜。'}</p>
  ...
  function showRules() {
    showChoiceDialog({
      title: '线上版核心规则',
      message: '每回合只能执行1个动作。封港戒严：指定对手下一完整回合无法行动。东风火攻：随机烧毁对手1张手牌，其下一回合开始不自动补牌至3张。调营换防：与指定对手直接交换位置。徐都督令、赵游击令：选择自己1件镇海战备，强制交换对手1件镇海战备，交换后立即检查所有玩家，集齐4种不同镇海战备立即获胜。地牌开放地点，巡防沿线移动1格，探海取得当地镇海战备。动作结算后补牌并结束回合；也可选择1张手牌换牌后结束。',
      choices: [],
      cancelText: '知道了'
    });
  }
